...
def generate_stacked_dataset(data_folder, X_train, y_train, X_test, y_test, imp='mean', resample=None):
    ...
    ds_train = PatientDataset(data_folder, X_train, y_train, model_type='RF', imputation=imp, resample_rate=resample)
    loader_train = DataLoader(ds_train, batch_size=128,
                              sampler=ImbalancedDatasetSampler(ds_train), num_workers=6)

    ds_test = PatientDataset(data_folder, X_test, y_test, model_type='RF', imputation=imp, resample_rate=resample)
    loader_test = DataLoader(ds_test, batch_size=128, num_workers=6)
        ...
        for pred in pred_list:
            for imp in imp_list:
                for resample_rate in rr_list:
                    ...
                    clf = RandomForestClassifier(max_depth=tree_depth, random_state=seed)
                    train_time_start = time.time()
                    clf.fit(X_train, y_train)
                    train_time_total = time.time() - train_time_start

                    save_model_path = os.path.join('models/RF/', 'sensitive_test')
                    Path(save_model_path).mkdir(parents=True, exist_ok=True)
                    save_model_path = save_model_path + f'pred{pred}-imp{imp}-sampling{resample_rate}-raw-{seed}.sav'
                    pickle.dump(clf, open(save_model_path, 'wb'))
                    with open(os.path.join(save_model_path, 'train_time.txt'), 'a') as train_time_file:
                        train_time_file.write(f'pred{pred}-imp{imp}-sampling{resample_rate}-raw-{seed}: {train_time_total}\n')

                    save_path = os.path.join('models/RF/', 'sensitive_test')
                    ...
                    df.to_csv(os.path.join(save_path, f'{pred}-{imp}-{resample_rate}-{seed}.csv'), sep=';')
                    ...
                    # np.save(f'models/RF/pred{pred}-imp{imp}-sampling{resample_rate}-raw-shap.npy', shap_values)