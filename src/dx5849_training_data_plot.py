...
save_path = 'processed/icd9_5849/dx_pred_' + str(pred_n_hour) + '_' + str(input_n_hour)
    ...
    for di, dx_ts in enumerate(dx_events_ts):
        ...
        for ti, (table, uid_range) in enumerate(uid_tables.items()):
            ...
            for uid in select_uid:
                ...
                ax[ti // 3][ti % 3].set_title(table, fontsize=12)
                ax[ti // 3][ti % 3].set_xlabel('Time after admission[h]', fontsize=10)
                ax[ti // 3][ti % 3].legend(loc='upper right', fontsize=8, borderaxespad=0.)
                # ax[ti // 3][ti % 3].legend(loc='center right', fontsize=12, bbox_to_anchor=(1.15, 0.5), borderaxespad=0.)

        fig.savefig(os.path.join(save_path, str(pid)+'_'+str(dx_ts)+'.png'), transparent=False, bbox_inches='tight')
        plt.cla()
        plt.close()
