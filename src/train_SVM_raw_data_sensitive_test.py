        ...
        for pred in pred_list:
            for imp in imp_list:
                for resample_rate in rr_list:
                    ...
                    save_path = os.path.join('models/SVM/', 'sensitive_test')
                    Path(save_path).mkdir(parents=True, exist_ok=True)
                    save_model_path = os.path.join(save_model_path, f'pred{pred}-imp{imp}-sampling{resample_rate}-raw-{seed}.sav')
                    pickle.dump(clf, open(save_model_path, 'wb'))
                    with open(os.path.join(save_path, 'train_time.txt'), 'a') as train_time_file:
